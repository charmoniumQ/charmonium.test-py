# See https://github.com/atrisovic/dataverse-r-study/blob/master/docker/exec_r_files.R

library(stringr)
library(R.utils)
library(reticulate)

main_dir = commandArgs(trailingOnly=TRUE)[1]
setwd(main_dir)
...
for (r_file in r_files) {
	...
	save(main_dir, r_files, r_file, filename,
	...
	local_vars <- c('main_dir', 'arg_temp', 'local_args', 'local_vars', 'external_vars', 'get_readability_metrics', 'error', 'filename', 'r_file', 'r_files', 'new_log_data', 'noauth', 'planb')
	external_vars <- ls()
	external_vars <- external_vars[!external_vars %in% local_vars]

	# Use python from the $PATH (default)
